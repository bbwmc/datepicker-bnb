...
export default {
  ...
  computed: {
    ...
    tooltipMessageDisplay() {
      return this.tooltipMessage
        ? this.tooltipMessage
        : `${this.nightsCount} ${
            this.nightsCount !== 1 ? this.options.i18n.nights : this.options.i18n.night
          }`;
    },
    showTooltip() {
      return (
    ...
    lineClass() {
      const date = fecha.format(this.date, "ddd");

      // if (date == "Sat") {
      //   return "last-day-line";
      // }
      // if (date == "Sun") {
      ...
      if (this.belongsToThisMonth) {
        ...
        if (this.options.allowedRanges.length !== 0) {
          if (!this.isDisabled && this.checkIn !== null && this.checkOut == null) {
            ...
            if (
              !this.allowedCheckoutDays.some(i => this.compareDay(i, this.date) == 0) &&
              this.isHighlighted
            ) {
  ...
  methods: {
    ...
    getDay(date) {
      console.log(fecha.format(date, "ddd"));
      return fecha.format(date, "ddd");
    },
    ...
    createAllowedCheckoutDays(date) {
      ...
      this.options.allowedRanges.forEach(i => this.allowedCheckoutDays.push(this.addDays(date, i)));
...
<style scoped>
.last-day-line .datepicker__month-day {
